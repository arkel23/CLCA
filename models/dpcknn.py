...
__all__ = ['DPCKNNViT']  # model_registry will add each entrypoint fn to this
...
class DPCKNNViT(nn.Module):
    ...
    def __init__(
        ...
        self.cluster_count = getattr(args, 'keep_rate', [0.9])
        self.cluster_loc = getattr(args, 'reduction_loc', [9])
        self.k_neighbors = getattr(args, 'k_neighbors', 5)
        ...
        if len(self.cluster_count) == 1:
            ...
            # dpcknn uses knn with k=5 so minimum value if subsequent reductions is 5
            for i, n in enumerate(self.cluster_count):
                if ((i + 1) != len(self.cluster_count)) and n <= 5:
                    self.cluster_count[i] = 5
                elif ((i + 1) == len(self.cluster_count)) and n == 0:
        ...
        if clc_recover_at_last:
            self.recovery_layers = self.cluster_loc + [depth - 2]
        else:
            self.recovery_layers = self.cluster_loc
        if self.clc:
            num_clr = getattr(args, 'num_clr', 0)
            print(f'Cross-layer cache with {num_clr} tokens and {self.clc_include_gap} gap')
            assert num_clr or self.clc_include_gap, 'Either num_clr or cla_include_gap if cla'
    ...
    def reset_classifier(self, num_classes: int, global_pool = None) -> None:
        if self.ifa_head:
            ...
            inter_feats = (len(self.cluster_loc) + 1)
    ...
    def add_clr(self, num_clr=1, init_cls=True) -> None:
        ...
        if init_cls:
           with torch.no_grad():
                ...
                print(f'Loaded {num_clr} CLC (cross-layer cache/carrier) tokens from CLS token')
    ...
    def get_new_module_names(self):
        if self.ifa_head is not False:
            return ['ifa_head', 'cluster_layers']
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        num_clr_tokens = self.num_clr if hasattr(self, 'num_clr') else 0

        # init token dict
        B, N, _ = x.shape
        N = N - self.num_prefix_tokens - num_clr_tokens
        device = x.device
        ...
        for i, blk in enumerate(self.blocks):
            if i in self.cluster_loc:
                global_tokens = x[:,:self.num_prefix_tokens]
                if num_clr_tokens:
                    suffix_tokens = x[:, -num_clr_tokens:]
                    x, idx_token, agg_weight, idx_centers, idx_cluster, cluster_centers = self.cluster_layers[cluster_cnt](x[:,self.num_prefix_tokens:-num_clr_tokens], idx_token, agg_weight, self.viz_mode)
                    x = torch.cat((global_tokens, x, suffix_tokens), dim=1)
                else:
                    x, idx_token, agg_weight, idx_centers, idx_cluster, cluster_centers = self.cluster_layers[cluster_cnt](x[:,self.num_prefix_tokens:], idx_token, agg_weight, self.viz_mode)
            ...
            if self.clc and (i in self.recovery_layers):
                ...
                cross_layer_cache = []
                num_clr_tokens += prev_feats.shape[1]
            ...
            if cls_list is not None and (i in self.cluster_loc or i == len(self.blocks) - 1):
                cls_list.append(x[:, 0])
...
@torch.no_grad()
def _load_weights(model: DPCKNNViT, checkpoint_path: str, prefix: str = '') -> None:
...
default_cfgs = {
    ...
    'dpcknn_vit_base_patch16_224.orig_in21k': _cfg(
    ...
    'dpcknn_vit_small_patch16_224.dino': _cfg(
        url='https://dl.fbaipublicfiles.com/dino/dino_deitsmall16_pretrain/dino_deitsmall16_pretrain.pth',
        hf_hub_id='timm/vit_small_patch16_224.dino',
        mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD, num_classes=0),
    'dpcknn_vit_base_patch16_224.dino': _cfg(
    ...
    'dpcknn_vit_small_patch14_reg4_dinov2.lvd142m': _cfg(
        ...
        mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD, num_classes=0,
        input_size=(3, 518, 518), crop_pct=1.0),
    'dpcknn_vit_base_patch14_reg4_dinov2.lvd142m': _cfg(
    ...
    'dpcknn_flexivit_base.1200ep_in1k': _cfg(
        ...
        input_size=(3, 240, 240), crop_pct=0.95),
    'dpcknn_flexivit_large.1200ep_in1k': _cfg(
        ...
        input_size=(3, 240, 240), crop_pct=0.95),
    'dpcknn_flexivit_base.patch16_in21k': _cfg(
...
@register_model
def dpcknn_vit_tiny_patch16_224(pretrained: bool = False, **kwargs):
    """ ViT-Tiny (Vit-Ti/16)
    """
    model_args = dict(patch_size=16, embed_dim=192, depth=12, num_heads=3)
    model = _create_vision_transformer('dpcknn_vit_tiny_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_vit_small_patch16_224(pretrained: bool = False, **kwargs):
    """ ViT-Small (ViT-S/16)
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6)
    model = _create_vision_transformer('dpcknn_vit_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_vit_small_patch8_224(pretrained: bool = False, **kwargs):
    """ ViT-Small (ViT-S/8)
    """
    model_args = dict(patch_size=8, embed_dim=384, depth=12, num_heads=6)
    model = _create_vision_transformer('dpcknn_vit_small_patch8_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_vit_base_patch16_224(pretrained: bool = False, **kwargs):
    """ ViT-Base (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 224x224, source https://github.com/google-research/vision_transformer.
    """
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12)
    model = _create_vision_transformer('dpcknn_vit_base_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_vit_base_patch8_224(pretrained: bool = False, **kwargs):
    """ ViT-Base (ViT-B/8) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 224x224, source https://github.com/google-research/vision_transformer.
    """
    model_args = dict(patch_size=8, embed_dim=768, depth=12, num_heads=12)
    model = _create_vision_transformer('dpcknn_vit_base_patch8_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_vit_large_patch16_224(pretrained: bool = False, **kwargs):
    """ ViT-Large model (ViT-L/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 224x224, source https://github.com/google-research/vision_transformer.
    """
    model_args = dict(patch_size=16, embed_dim=1024, depth=24, num_heads=16)
    model = _create_vision_transformer('dpcknn_vit_large_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_vit_large_patch14_224(pretrained: bool = False, **kwargs):
    """ ViT-Large model (ViT-L/14)
    """
    model_args = dict(patch_size=14, embed_dim=1024, depth=24, num_heads=16)
    model = _create_vision_transformer('dpcknn_vit_large_patch14_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_vit_huge_patch14_224(pretrained: bool = False, **kwargs):
    """ ViT-Huge model (ViT-H/14) from original paper (https://arxiv.org/abs/2010.11929).
    """
    model_args = dict(patch_size=14, embed_dim=1280, depth=32, num_heads=16)
    model = _create_vision_transformer('dpcknn_vit_huge_patch14_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_vit_base_patch16_224_miil(pretrained: bool = False, **kwargs):
    """ ViT-Base (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    Weights taken from: https://github.com/Alibaba-MIIL/ImageNet21K
    """
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, qkv_bias=False)
    model = _create_vision_transformer(
        'dpcknn_vit_base_patch16_224_miil', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_vit_medium_patch16_gap_240(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'dpcknn_vit_medium_patch16_gap_240', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_vit_medium_patch16_gap_256(pretrained: bool = False, **kwargs):
    ...
    model_args = dict(
        ...
        global_pool='avg', qkv_bias=False, init_values=1e-6, fc_norm=False)
    model = _create_vision_transformer(
        'dpcknn_vit_medium_patch16_gap_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_vit_base_patch16_gap_224(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'dpcknn_vit_base_patch16_gap_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_vit_huge_patch14_gap_224(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'dpcknn_vit_huge_patch14_gap_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_vit_giant_patch16_gap_224(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'dpcknn_vit_giant_patch16_gap_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_vit_base_patch16_clip_224(pretrained: bool = False, **kwargs):
    """ ViT-B/16 CLIP image tower
    """
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, pre_norm=True, norm_layer=nn.LayerNorm)
    model = _create_vision_transformer(
        'dpcknn_vit_base_patch16_clip_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_flexivit_small(pretrained: bool = False, **kwargs):
    """ FlexiViT-Small
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, no_embed_class=True)
    model = _create_vision_transformer('dpcknn_flexivit_small', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_flexivit_base(pretrained: bool = False, **kwargs):
    """ FlexiViT-Base
    """
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, no_embed_class=True)
    model = _create_vision_transformer('dpcknn_flexivit_base', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_flexivit_large(pretrained: bool = False, **kwargs):
    """ FlexiViT-Large
    """
    model_args = dict(patch_size=16, embed_dim=1024, depth=24, num_heads=16, no_embed_class=True)
    model = _create_vision_transformer('dpcknn_flexivit_large', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_vit_small_patch14_dinov2(pretrained: bool = False, **kwargs):
    """ ViT-S/14 for DINOv2
    """
    model_args = dict(patch_size=14, embed_dim=384, depth=12, num_heads=6, init_values=1e-5, img_size=518)
    model = _create_vision_transformer(
        'dpcknn_vit_small_patch14_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_vit_base_patch14_dinov2(pretrained: bool = False, **kwargs):
    """ ViT-B/14 for DINOv2
    """
    model_args = dict(patch_size=14, embed_dim=768, depth=12, num_heads=12, init_values=1e-5, img_size=518)
    model = _create_vision_transformer(
        'dpcknn_vit_base_patch14_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_vit_large_patch14_dinov2(pretrained: bool = False, **kwargs):
    """ ViT-L/14 for DINOv2
    """
    model_args = dict(patch_size=14, embed_dim=1024, depth=24, num_heads=16, init_values=1e-5, img_size=518)
    model = _create_vision_transformer(
        'dpcknn_vit_large_patch14_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_vit_giant_patch14_dinov2(pretrained: bool = False, **kwargs):
    """ ViT-G/14 for DINOv2
    """
    ...
    model = _create_vision_transformer(
        'dpcknn_vit_giant_patch14_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_vit_small_patch14_reg4_dinov2(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'dpcknn_vit_small_patch14_reg4_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_vit_base_patch14_reg4_dinov2(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'dpcknn_vit_base_patch14_reg4_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_vit_large_patch14_reg4_dinov2(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'dpcknn_vit_large_patch14_reg4_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_vit_giant_patch14_reg4_dinov2(pretrained: bool = False, **kwargs):
    """ ViT-G/14 for DINOv2
    """
    # The hidden_features of SwiGLU is calculated by:
    # hidden_features = (int(hidden_features * 2 / 3) + 7) // 8 * 8
    ...
    model = _create_vision_transformer(
        'dpcknn_vit_giant_patch14_reg4_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_vit_base_patch16_siglip_224(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, class_token=False, global_pool='map',
    )
    model = _create_vision_transformer(
        'dpcknn_vit_base_patch16_siglip_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_vit_base_patch16_siglip_256(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, class_token=False, global_pool='map',
    )
    model = _create_vision_transformer(
        'dpcknn_vit_base_patch16_siglip_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_vit_base_patch16_siglip_512(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, class_token=False, global_pool='map',
    )
    model = _create_vision_transformer(
        'dpcknn_vit_base_patch16_siglip_512', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_vit_large_patch16_siglip_256(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=1024, depth=24, num_heads=16, class_token=False, global_pool='map',
    )
    model = _create_vision_transformer(
        'dpcknn_vit_large_patch16_siglip_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_vit_medium_patch16_reg4_256(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=512, depth=12, num_heads=8, class_token=True,
        no_embed_class=True, reg_tokens=4,
    )
    model = _create_vision_transformer(
        'dpcknn_vit_medium_patch16_reg4_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_vit_medium_patch16_reg4_gap_256(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=512, depth=12, num_heads=8,
        class_token=False, no_embed_class=True, reg_tokens=4, global_pool='avg',
    )
    model = _create_vision_transformer(
        'dpcknn_vit_medium_patch16_reg4_gap_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_vit_base_patch16_reg8_gap_256(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, class_token=False,
        no_embed_class=True, global_pool='avg', reg_tokens=8,
    )
    model = _create_vision_transformer(
        'dpcknn_vit_base_patch16_reg8_gap_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_deit_tiny_patch16_224(pretrained=False, **kwargs):
    """ DeiT-tiny model @ 224x224 from paper (https://arxiv.org/abs/2012.12877).
    ImageNet-1k weights from https://github.com/facebookresearch/deit.
    """
    model_args = dict(patch_size=16, embed_dim=192, depth=12, num_heads=3)
    model = _create_vision_transformer('dpcknn_deit_tiny_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_deit_small_patch16_224(pretrained=False, **kwargs):
    """ DeiT-small model @ 224x224 from paper (https://arxiv.org/abs/2012.12877).
    ImageNet-1k weights from https://github.com/facebookresearch/deit.
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6)
    model = _create_vision_transformer('dpcknn_deit_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_deit_base_patch16_224(pretrained=False, **kwargs):
    """ DeiT base model @ 224x224 from paper (https://arxiv.org/abs/2012.12877).
    ImageNet-1k weights from https://github.com/facebookresearch/deit.
    """
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12)
    model = _create_vision_transformer('dpcknn_deit_base_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_deit3_small_patch16_224(pretrained=False, **kwargs):
    ...
    model = _create_vision_transformer('dpcknn_deit3_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_deit3_medium_patch16_224(pretrained=False, **kwargs):
    ...
    model = _create_vision_transformer('dpcknn_deit3_medium_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_deit3_base_patch16_224(pretrained=False, **kwargs):
    ...
    model = _create_vision_transformer('dpcknn_deit3_base_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_deit3_large_patch16_224(pretrained=False, **kwargs):
    ...
    model = _create_vision_transformer('dpcknn_deit3_large_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def dpcknn_deit3_huge_patch14_224(pretrained=False, **kwargs):
    ...
    model = _create_vision_transformer('dpcknn_deit3_huge_patch14_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model
