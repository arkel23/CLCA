...
__all__ = ['ViT']  # model_registry will add each entrypoint fn to this
...
class ViT(nn.Module):
    ...
    def __init__(
        ...
        # default reduction locs
        self.reduction_loc = [3, 6, 9]
    ...
    def no_weight_decay(self) -> Set:
        return {'pos_embed', 'cls_token', 'dist_token', 'clc'}
        ...
        if self.ifa_head:
            ...
            if self.kernels_per_layer:
                ...
                conv = nn.Sequential(
                    ...
                    Rearrange('b c 1 -> b c'),
                    # norm usually before act (but results dont change by much) 
                    nn.ReLU(),
                    nn.LayerNorm(self.embed_dim),
...
@torch.no_grad()
def _load_weights(model: ViT, checkpoint_path: str, prefix: str = '') -> None:
...
default_cfgs = {
    ...
    'vit_small_patch16_224.in1k_mocov3': _cfg(
    ...
    'vit_base_patch16_224.in1k_mocov3': _cfg(
...
def deit_tiny_patch16_224(pretrained=False, **kwargs):
    ...
    """
    model_args = dict(patch_size=16, embed_dim=192, depth=12, num_heads=3)
    model = _create_vision_transformer('deit_tiny_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model
...
def deit_small_patch16_224(pretrained=False, **kwargs):
    ...
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6)
    model = _create_vision_transformer('deit_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model
...
def deit_base_patch16_224(pretrained=False, **kwargs):
    ...
    """
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12)
    model = _create_vision_transformer('deit_base_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model
...
def deit3_huge_patch14_224(pretrained=False, **kwargs):
    ...
    model = _create_vision_transformer('deit3_huge_patch14_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model
